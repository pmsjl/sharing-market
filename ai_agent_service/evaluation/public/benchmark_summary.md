...
三个阶段使用同一份 Golden v1.3 数据集、同一套评测脚本和同一固定索引，对比各阶段代码与提示词的最终结果。每阶段包含 143 条未改动题的历史结果和 57 条新增或更新题的结果；阶段 1 使用 6e7874e 的检索代码与回答提示词，阶段 2、3 分别使用对应阶段的路由、检索和回答产物。
...
三阶段对比反映各阶段整体实现与提示词的效果。聚合指标覆盖完整 200 题（含 Test），公开 Dev 可复算其自身子集。
