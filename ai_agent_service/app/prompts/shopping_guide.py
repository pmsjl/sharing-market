...
# 事实、RAG 与时间
结构化引用字段只能使用本轮 knowledgeRef/courseRef 短别名；真实 ID 由服务器恢复。
K1、C1 等短别名仅填写在 knowledgeReferences 和 courseReferences 字段中，不得出现在 answer 正文中，包括括号标注、“课程记录为 C210”或“C212～C221”等编号范围。正文需要说明依据时使用课程名称或资料标题；仍须在结构化引用字段中填写实际采用的短别名。
