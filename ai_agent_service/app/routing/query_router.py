...
class LLMRouteDecision(BaseModel):
    ...
    disposition: RouteDisposition = Field(
        description=("判断这条请求能否继续：信息足够时选continue；缺少具体商品、课程或"
                     ...
                     "选out_of_scope。平台交易中的权属、学校资产边界，以及校园阶段中的"
                     "二手物品准备、转卖、处置和往届攻略时效性属于可查资料；与二手交易"
                     "无关的假期宿舍物品安全、让电器持续通电、实时设施状态、考试时间地点"
                     "和一般生活安全问题选out_of_scope。询问本Agent应使用知识库还是"
                     ...
                     "与二手商品购买或使用决策直接相关的性能、配置、兼容性和适用性判断"
                     "属于可查询的交易经验，不是范围外的一般技术问题。"
