...
class LLMRouteDecision(BaseModel):
    ...
    disposition: RouteDisposition = Field(
        description=("判断这条请求能否继续：信息足够时选continue；缺少具体商品、课程或"
                     "使用条件时选clarify；与校园二手交易、平台规则及相关购买决策无关时"
                     "选out_of_scope。平台交易中的权属、学校资产边界，以及校园阶段中"
                     "明确涉及个人物品购买、二手取得、转卖、转赠、处置或适配时机的请求"
                     "属于可查资料；单纯的新生报到手续、往届报到安排、假期宿舍物品安全、"
                     "让电器持续通电、实时设施状态、考试时间地点和一般生活安全问题选"
                     "out_of_scope。询问本Agent应使用知识库还是"
                     "实时工具属于平台能力说明，不是范围外问题。兼容性核验、准备顺序等"
                     "通用方法，以及已经给出书名版本后询问是否适合买来上课、如何降低"
                     "买错风险，都可以继续查询并给条件化建议；只有用户要求某门课程的"
                     "指定版本、必须购买等确定结论且课程仍无法识别时才需要追问课程。"
                     "只有当前消息或可靠会话上下文明确涉及购买、二手商品选择、验货或"
                     "具体商品使用决策时，与该商品直接相关的性能、配置、兼容性和适用性"
                     "判断才属于可查询的交易经验。没有商品决策上下文的编程、硬件性能、"
                     "系统配置和一般技术问答选out_of_scope；不得仅因出现内存、系统、"
                     "兼容性、型号等词推断购物意图。"
                     "资料中可能没有答案不代表用户信息不足，此时仍选continue。"), )
...
- 只有当前消息或可靠会话上下文明确出现购买、二手商品选择、验货或具体商品使用决策时，性能、配置、兼容性和适用性问题才属于范围内；纯编程、硬件性能和系统配置问题选out_of_scope，不能自行补出购物意图。
- 校园阶段资料只有在问题明确涉及个人物品购买、二手取得、转卖、转赠、处置或适配时机时才属于范围内；单纯的新生报到手续、往届报到安排、假期安全和其他一般校园事务选out_of_scope。
