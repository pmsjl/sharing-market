...
import { getRoleHomePath, isPathAllowedForRole } from "@/utils/roleHome";
...
router.beforeEach(async (to, from, next) => {
  ...
  if (token) {
    ...
    if (userRole) {
      ...
      if (!isPathAllowedForRole(to.path, userRole)) {
        next({ path: getRoleHomePath(userRole), replace: true });
        return;
      }
